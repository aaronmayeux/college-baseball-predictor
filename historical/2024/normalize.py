"""Parse cached 2024 Warren Nolan schedules and reconcile reciprocal observations.
...
def parse(t):
 ...
 for attrs,body in re.findall(r'<li\b([^>]*)>(.*?)</li>',s,re.S):
  ...
  try:date=datetime.date(2024,list(calendar.month_abbr).index(month.title()),int(day)).isoformat()
  except Exception:date=None
  opp=first(body,r'opp-line-link"[^>]*>(.*?)</a>') or first(body,r'team-schedule__opp-line">(.*?)</span>')
  oppid=re.search(r'opp-line-link"[^>]+href="/baseball/2024/schedule/([^"/]+)"',body)
  ...
  elif 'Tournament' in label and date and date>='2024-05-01':stage='conference_tournament'
  elif date and date>='2024-05-31':stage='late_season_unclassified'
