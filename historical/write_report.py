"""Generate the human-readable audit report from the saved validation and metrics."""
import json,pathlib
ROOT=pathlib.Path(__file__).resolve().parent

def read(n):return json.loads((ROOT/n).read_text())
def run():
 audits=read('coverage_validation.json');metrics=read('baseline_metrics.json');cuts=read('cutoff_summary.json')
 counts='\n'.join(f"| {a['season']} | {a['provider_teams']} | {a['active_d1_teams']} | {a['games']:,} | {a['record_matches']}/{a['provider_teams']} |" for a in audits)
 rows=[r for r in metrics if r['mode']=='regular_only' and r['season']>2021]
 mt='\n'.join(f"| {r['season']} | {r['elo']['n']} | {r['elo']['accuracy']:.1%} | {r['win_rate']['accuracy']:.1%} | {r['elo']['log_loss']:.4f} | {r['win_rate']['log_loss']:.4f} | {r['elo']['brier']:.4f} |" for r in rows)
 ct='\n'.join(f"| {y} | {read('cutoffs.json')['seasons'][str(y)]['forecast_cutoff']} | {next(r['eligible'] for r in cuts if r['season']==y and r['mode']=='regular_only'):,} | {next(r['eligible'] for r in cuts if r['season']==y and r['mode']=='conference_inclusive'):,} |" for y in range(2021,2026))
 alt='\n'.join(f"| {y} | "+' | '.join(f"{next(r for r in metrics if r['season']==y and r['mode']==mode and r['subset']=='ncaa')['elo']['log_loss']:.4f}" for mode in ['regular_only','conference_inclusive','daily_updated'])+' |' for y in range(2022,2026))
 text=f'''# College Baseball Predictor — Historical Coverage and Elo Baseline
...
'''
 (ROOT/'College_Baseball_Historical_Coverage_and_Elo_Report.md').write_text(text)
