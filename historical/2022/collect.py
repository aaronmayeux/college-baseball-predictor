"""2022 results coverage pilot. Cached raw pages; no model fitting."""
...
def roster():
 b,m=fetch('rpi.html','https://www.warrennolan.com/baseball/2022/rpi-live');s=re.sub(r'<!--.*?-->','',b.decode('utf-8',errors='replace'),flags=re.S);out=[]
 for row in re.findall(r'<tr\b[^>]*>(.*?)</tr>',s,re.S):
  a=re.search(r'<a[^>]+href="/baseball/2022/schedule/([^"/]+)"[^>]*>([^<]+)</a>',row)
  if not a:continue
  ...
  out.append({'team_id':'wn:'+a[1],'slug':a[1],'name':clean(a[2]),'season':2022,'conference':re.sub(r'\s*\([\d-]+\)$','',clean(conf[1])) if conf else None,'reference_record':rec})
 assert len({x['slug'] for x in out})==len(out) and len(out)>290
 (ROOT/'teams.json').write_text(json.dumps(out,indent=2));return out

def download(t):
 time.sleep(.3)
 _,m=fetch(t['slug']+'.html','https://www.warrennolan.com/baseball/2022/schedule/'+t['slug']);return t['slug'],m
