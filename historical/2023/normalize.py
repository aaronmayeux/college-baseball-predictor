"""Parse cached 2023 Warren Nolan schedules and reconcile reciprocal observations.
...
def parse(t):
 ...
 for attrs,body in re.findall(r'<li\b([^>]*)>(.*?)</li>',s,re.S):
  ...
  try:date=datetime.date(2023,list(calendar.month_abbr).index(month.title()),int(day)).isoformat()
  except Exception:date=None
  opp=first(body,r'opp-line-link"[^>]*>(.*?)</a>') or first(body,r'team-schedule__opp-line">(.*?)</span>')
  oppid=re.search(r'opp-line-link"[^>]+href="/baseball/2023/schedule/([^"/]+)"',body)
  ...
  elif 'Tournament' in label and date and date>='2023-05-01':stage='conference_tournament'
  elif date and date>='2023-06-02':stage='late_season_unclassified'
...
def run():
 ...
 for gid,rs in groups.items():
  ...
  game={'game_id':('wn:2023:' if a['provider']=='warren_nolan' else 'official:2023:')+gid,'season':2023,'game_date':a['game_date'],'team_a_id':a['team_id'],'team_b_id':a['opponent_id'],'runs_a':a['runs_for'],'runs_b':a['runs_against'],'tie':a['outcome']=='T','stage':a['stage'],'stage_basis':a['stage_basis'],'source_event_label':a['source_event_label'],'venue_text':a['venue_text'],'actual_home_team_id':None,'venue_verification':'unverified','timing_review':any(r.get('timing_review',False) for r in rs),'score_bases':sorted(set(r.get('score_basis','result_text') for r in rs)),'observation_count':len(rs),'reciprocal_check':'pass' if not errs else 'review','issues':errs,'corrections':[r['correction'] for r in rs if 'correction' in r],'distinct_evidence_record_count':1 if a['provider'].startswith('official_') else len(rs),'observation_refs':[{'team_id':r['team_id'],'source_url':r['source_url'],'raw_path':r['raw_path'],'raw_sha256':r['raw_sha256'],'retrieved_at':r['retrieved_at']} for r in rs]}
  games.append(game)
