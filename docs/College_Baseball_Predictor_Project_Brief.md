...
The separate pitching extractor now verifies BF and explicit starts across all 112 Davidson/Missouri State boxes and produces complete cutoff-specific workload/role summaries. Missing pitch counts and actual work dates prevent rest claims. Next: qualify **LSU pitcher BF and explicit starter/relief evidence** in its cached StatCrew boxes; never infer starts from table order. The qualification plan owns the subsequent targeted multi-season expansion and fair evaluation; depth/ace thresholds and scaling remain open.
