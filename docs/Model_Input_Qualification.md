...
The retained Arkansas index links to the required count reports but does not contain them; the sample-only result below identifies the next evidence gap. Establish acceptable source-specific season scope before any additional collection. The structured team-log result below does not qualify pitcher histories or weaken appearance gates.
...
`retained_scope` in the preflight output records linked-report retention, exact cached box URLs and missing season-box counts. Successful metadata without matching source bytes fails; a link or failed response never counts as retained evidence. Full-season appearance qualification stays false even when the sample succeeds. To progress, review acceptable access for the two exact Arkansas count-report URLs before any bounded retrieval, then reconcile them if available. Season-wide box collection remains uncleared. No provider contact, request, outcome metric, model/app adjustment or new evidence archive occurred.
