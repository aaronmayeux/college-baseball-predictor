...
Milestone 6 has a focused qualification plan and offline hitting and pitching extractors, including retained team game logs verified September 26, 2026. No richer-count feature weights or quality-arm thresholds are selected. The separate [team-run experiment](Team_Run_Experiment.md) has now tested scoring/run-prevention additions on all 2021–2024 tournament fields; its mixed results leave the app unchanged. The [triage](Statistics_Discovery_and_Triage.md) owns the broader inventory; [SPEC](../historical/SPEC.md) owns cutoff and holdout rules.
...
The existing inventory targets the **64 NCAA field teams in each of 2021–2024 (256 team-seasons)**. It remains a full-field scope reference, not a statistically proven minimum or collection permission. The active source decision must establish a feasible team-level dataset and freeze its scope before scoring. If coverage requires a smaller predeclared experiment, revise the plan before inspecting candidate metrics and limit conclusions accordingly. Preserve the nationwide results for Elo; player histories and every opponent's season are unnecessary for initial team-level features.

### Retained four-team preflight (paused)

The **2022 Stillwater regional: Oklahoma State, Arkansas, Grand Canyon and Missouri State** was selected for reusable evidence and a complete four-team group, not feature performance. Its cached inventory contains 215 regular-only team-game rows (213 distinct games), or 232 conference-inclusive rows (230 games), and seven potential NCAA matchup targets. These are result inventories, not qualified feature rows or request counts. Preserve the completed findings below, but do not require this preflight's completion before the team-level experiment. It is too narrow for national validation.
...
Across the four seasons, conference-inclusive inputs cover 14,175 team-game rows in 12,072 distinct result games. The modes overlap; do not add their volumes. Request volume depends on the selected team-data route, cache reuse and reconciliation targets; this is not a player-box request budget. Existing appearance extractors retain their full-season gates and non-D1 workload evidence. A team-level adapter may qualify complete pre-cutoff team counts without reconstructing individual appearances; document its source-specific completeness and reconciliation checks. Final/postseason totals remain audit-only.
...
| Arkansas | 66/67 strict; 67/67 with separate completion annotation | 54/54; 56/56 after annotation | Both sides pass core batting/pitching counts; ten pitchers total. Both sides now pass extra-base/PA, pitcher BF components and explicit starting-role checks in the retained box |
...
The retained Arkansas index links to count reports it does not contain; that gap is paused under the current priority. The structured team-log result below does not qualify pitcher histories or weaken appearance gates.
...
### Arkansas retained-evidence limit and sample qualification

The preflight audit now checks the named team with the shared StatCrew hitting/pitching adapters. In the February 18, 2022 Illinois State–Arkansas box, both teams pass core and play-derived extra-base/HBP/sacrifice counts, opposing PA/BF, each pitcher's complete BF components and explicit lineup starter matching. Arkansas has 35 batting PA and 39 pitching BF; Illinois State has the reverse. All ten pitchers pass (five per team); Connor Noland and Jordan Lussier are explicitly identified as the starters. This verifies a sample, not a season or quality-arm label.

A defensive `out at home c to p` play previously resembled a pitching substitution. Changes now require the start of a sentence; removed replay comments and whitespace are normalized before parsing. All 68 LSU hitting outputs and the complete LSU pitching report remain identical to the prior adapters. No default team or unrelated school's lineup is substituted.

The hash-verified preflight cache contains **one box per new school**: Arkansas 1/67, Oklahoma State 1/64 and Grand Canyon 1/62. These counts describe that cache only, not all possible shared opponent boxes or national coverage. Arkansas's index links to `teamgbg.htm` (dated team counts) and `teamcume.htm` (season audit totals), but neither response is retained there. The supplied complete older archives also contain no Arkansas-hosted 2022 count reports; an opponent-hosted Arkansas matchup does not fill a season. No full-season Arkansas hitting rates or new-school pitcher histories are qualified. The existing Oklahoma State/Grand Canyon team-log hitting qualification is unchanged.

`retained_scope` in the preflight output records linked-report retention, exact cached box URLs and missing season-box counts. Successful metadata without matching source bytes fails; a link or failed response never counts as retained evidence. Full-season appearance qualification stays false even when the sample succeeds. Retrieving these reports is deferred unless the focused team-source decision makes them necessary. Season-wide box collection remains uncleared. No provider contact, request, outcome metric, model/app adjustment or new evidence archive occurred.

### Fitting and promotion boundaries

For the new team-component experiment, follow the [current practical contract](../historical/SPEC.md#practical-team-component-experiment--current-priority). Aaron superseded the earlier fixed four-season/full-coverage and regular-only-selection prerequisites: use supported dated reports, explicitly include conference tournaments, quantify minor gaps, retain Elo fallback, and lock a chronological plan and small candidate set before scoring. Existing strict extractor checks remain reproducible evidence, not prerequisites for a new national-snapshot adapter. The closed net-run protocol is unchanged.
...
[DATA.md](DATA.md#offline-hitting-input-pilot) owns restoration and both extraction commands. Repeated outputs are byte-identical. The prior richer-input implementation had **182 tests** passing (152 scripts, 30 historical; seven additional generic-team, fielding-play and retained-cache regressions), including 13 hitting tests for count formulas, unknown denominators, interference, replay comments, pitch-code confusion, missing/duplicate events, incomplete modes and chronological exclusions. Twelve pitching tests cover BF/interference, role flags versus generic lineup starts/order, missing counts, zero-out appearances, rates, incomplete modes and cutoff exclusions. Eight LSU tests additionally check explicit starter matching, ambiguity, event allocation, batting-slot changes, BF mismatches and cumulative interference/start reconciliation. Tests need no external data.
